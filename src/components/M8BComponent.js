...
import { ballAPI } from '../DataService';
...
function RiAComponent() {
    ...
    const getOutput = async () => {
        const response = await ballAPI(num1);
        setOut(response);
    }
    return (
        <>
                    ...
                    <div className="bg-grey h-fit w-fot flex justify-center p-5">
                        <div className="w-full text-[48px] text-center text-white p-4">
                            <form className="flex flex-col items-center">
                                <h1><label for="input1" className="p-5">Input:</label><input type="text" onChange={(e) => { setNum1(e.target.value) }} id="input1" name="input1" placeholder="your question" className='text-black' /></h1>
                            </form>
