...
import { restAPI } from '../DataService';

function ResPComponent() {
    const [num1, setNum1] = useState(0);
    const [output, setOut] = useState('');

    const getOutput = async () => {
        const response = await restAPI(num1);
        setOut(response);
    }
    return (
        <>
                    ...
                    <div className="bg-grey h-fit w-fot flex justify-center p-5">
                        <div className="w-full text-[48px] text-center text-white p-4">
                            <form className="flex flex-col items-center">
                                <h1><label for="input1" className="p-5">Input:</label><input onChange={(e) => { setNum1(e.target.value) }} type="text" id="input1" name="input1" placeholder="fastfood/sitdown/takeout" className='text-black' /></h1>
                            </form>
