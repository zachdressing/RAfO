...
import { wakeAPI } from '../DataService';
...
function WUComponent() {
    ...
    const getOutput = async () => {
        const response = await wakeAPI(num1, num2);
        setOut(response);
    }
                    ...
                    <div class="bg-grey h-fit w-fit flex justify-center p-5">
                        <div class="w-full text-[48px] text-center text-white p-4">
                            <form class="flex flex-col justify-center items-center">
                                <h1><label for="input1" class="p-5">Input:</label><input onChange={(e) => { setNum1(e.target.value) }} type="text" className='text-black' id="input1" name="input1" placeholder="your name" /></h1>
                                <h1><label for="input2" class="p-5">Input:</label><input onChange={(e) => { setNum2(e.target.value) }} type="text" className='text-black' id="input2" name="input2" placeholder="your time" /></h1>
                            </form>
